...
基于 DNSPod 用户 API 实现的纯 Shell 动态域名客户端。IPv4 优先适配网卡地址，无法获得合法外网地址则使用 DNSPod 接口自动更新；~~为保证成功率，IPv6 直接从外部接口获取地址。~~ 实测群晖系统不明原因无法正常访问IPv6回显网站，并且外部网站可能不稳定，现已改成通过ip addr配合正则匹配，仅匹配GUA地址。
